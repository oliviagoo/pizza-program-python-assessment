...
def welcome():
    ...
    print("We will guide you through the process as you decide whether to pick up your piping hot pizza or have it delivered directly to your doorstep, and choose from our delicious menu of pizzas.")
    print()
...
def confirm(order, stuff_crust, price, order_type, address, name):
    ...
    while True:
        ...
        if correct == "n":
            reorder = force_yn("Are you sure you want to continue shopping? (Y or N) ")
            print()
            if reorder == "y":
...
def delivery(price):
    ...
    while True:
        answer = input("Would you like to get your pizza delivered for ${:.2f} delivery fee (D), or pick it up? (P) ".format(DELIVERY_PRICE)).lower().strip()
        if answer == "d":
            price += DELIVERY_PRICE
...
def edit_order(order, stuff_crust, price, order_type):
    output_order(price, order, stuff_crust, order_type)
    while True:
        remove = force_int("Enter the number of the pizza you'd like to remove, or press 0 to quit: ") - 1
        if order[-1] < remove:
...
def order_pizza(price, order, stuff_crust):
    ...
    PIZZA_RESTRICTION = 3
    while True:
        pizza_order = force_int("Enter the ID of the pizza you want to order, or press 0 to quit: ")
        if pizza_order != 0:
...
def stuffed_crust(price, stuff_crust):
    answer = force_yn("Would you like to add stuffed crust for ${:.2f}? (Y or N?) ".format(STUFF_PRICE))
    if answer == "y":
        price += STUFF_PRICE
    print()
    return answer, price
            ...
            if order[pizza] < gourmet_menu[0][2]:
                for option in regular_menu:
                    if option[2] == order[pizza]:
                        print("{}) {} with stuffed crust - ${:.2f}".format(pizza + 1, option[0], REG_PRICE + STUFF_PRICE))
                        break
            else:
                for option in gourmet_menu:
                    if option[2] == order[pizza]:
                        print("{}) {} with stuffed crust - ${:.2f}".format(pizza + 1, option[0], GOURMET_PRICE + STUFF_PRICE))
                        break
        else:
            if order[pizza] < gourmet_menu[0][2]:  
                for option in regular_menu:
                    if option[2] == order[pizza]:
                        print("{}) {} - ${:.2f}".format(pizza + 1, option[0], REG_PRICE))
                        break
            else:
                for option in gourmet_menu:   
                    if option[2] == order[pizza]:
                        print("{}) {} - ${:.2f}".format(pizza + 1, option[0], GOURMET_PRICE))
                        break
    if order_type == "d":
...
def order_input(price, order_type, order, stuff_crust):
    while True:
        selection = input("Enter M to see the menu, P to order a pizza, O to see your order so far, E to edit your order, C to clear your order, or F to finish and place your order: ").lower().strip()
        print()
        if selection == "m":
            output_menu()
        elif selection == "p":
            ...
            if len(order) > 0:
                order, stuff_crust, price = edit_order(order, stuff_crust, price, order_type)
            else:
            ...
            confirmation = force_yn("Are you sure you want to clear your order? You will lose everything you have ordered so far. (Y or N?) ")
            if confirmation == "y":
                order = []
                stuff_crust = []
                if order_type == "d":
